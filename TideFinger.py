...
"""
version check
the python version should gt 3.7
"""

py_version = float(sys.version.split()[0][:3])
if py_version < 3.7:
    exit("Need python version 3.7.* or later")
...
r_header = re.compile(r'header="(.*)"')
r_body = re.compile(r'body="(.*)"')
...
class WhatCms:
    ...
    def request_url(url):
        try:
            ...
            r = requests.get(url=url, headers=requests_headers(), timeout=request_timeout, verify=False,
                             proxies=web_proxy)
            r.encoding = 'utf-8'
    ...
    def find_cms_with_file(self):
        ...
        while True:
            ...
            finger_id, cms_name, path, match_pattern, options, hit = eachline[0], eachline[1], eachline[2], eachline[3], \
                                                                     eachline[4], eachline[5]
...
class Cmsscanner(object):
    ...
    def check_rule(self, key, header, body, title):
        ...
        try:
            if 'title="' in key:
                ...
                if re.findall(r_body, key)[0] in body: return True
            else:
                if re.findall(r_header, key)[0] in header: return True
        except Exception as e:
            pass
    ...
    def handle(self, _id, header, body, title):
        ...
        # 需要同时满足条件的情况
            ...
            # 与条件下存在并条件: 1||2||(3&&4)
            if '&&' in re.findall(r_bracket, key)[0]:
                for rule in key.split('||'):
                    if '&&' in rule:
                        num = 0
